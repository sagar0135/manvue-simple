...
// Initialize products with fallback data
...
document.addEventListener('DOMContentLoaded', async function() {
    console.log('MANVUE initializing...');
    ...
    initializeTryOnProducts();
    
    console.log('MANVUE initialized successfully');
});
...
async function performMLTextSearch(query, category = 'all') {
    try {
        ...
        if (!response.ok) {
            throw new Error('ML search failed');
        }
        
        const results = await response.json();
        return results.results || [];
    } catch (error) {
        console.error('ML text search error:', error);
        // Fallback to local search
        return performLocalSearch(query, category);
    }
}

// AI-powered image search
async function performMLImageSearch(imageData) {
    try {
        const response = await fetch(`${API_BASE_URL}/products/image-search`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (!response.ok) {
            throw new Error('ML image search failed');
        }
        
        const results = await response.json();
        return results;
    } catch (error) {
        console.error('ML image search error:', error);
        return {
            success: false,
            error: error.message,
            similar_products: []
...
// Enhanced search function with ML integration
async function enhancedSearch(query, category = 'all') {
    showMessage('🔍 Performing AI-powered search...');
    
    try {
        // Try ML search first
        const mlResults = await performMLTextSearch(query, category);
        
        if (mlResults.length > 0) {
            showMessage(`✅ Found ${mlResults.length} products with AI search`);
            displayProducts(mlResults);
            return;
        }
    } catch (error) {
        console.log('ML search failed, falling back to local search');
    }
    
    // Fallback to local search
    const localResults = performLocalSearch(query, category);
    if (localResults.length > 0) {
        showMessage(`📋 Found ${localResults.length} products with local search`);
        displayProducts(localResults);
    } else {
        showMessage('❌ No products found. Try different keywords.');
    }
}

// Local search fallback
function performLocalSearch(query, category = 'all') {
        ...
        productDiv.innerHTML = `
            ...
            <div class="product-info">
                ...
                <div class="product-actions">
                    <button onclick="editProduct(${product.id})" class="btn btn-sm btn-primary">✏️ Edit</button>
                    <button onclick="deleteProduct(${product.id})" class="btn btn-sm btn-danger">🗑️ Delete</button>
                </div>
            </div>
        `;
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                showMessage(`✅ Product ${editingProductId ? 'updated' : 'created'} successfully`);
                hideProductForm();
                loadAdminProducts(); // Refresh the list
...
// This DOMContentLoaded event is now handled in the main initialization above
...
// Initialize try-on products functionality
function initializeTryOnProducts() {
